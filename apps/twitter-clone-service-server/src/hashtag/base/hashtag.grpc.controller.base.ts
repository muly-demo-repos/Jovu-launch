/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { GrpcMethod } from "@nestjs/microservices";
import { HashtagService } from "../hashtag.service";
import { HashtagCreateInput } from "./HashtagCreateInput";
import { HashtagWhereInput } from "./HashtagWhereInput";
import { HashtagWhereUniqueInput } from "./HashtagWhereUniqueInput";
import { HashtagFindManyArgs } from "./HashtagFindManyArgs";
import { HashtagUpdateInput } from "./HashtagUpdateInput";
import { Hashtag } from "./Hashtag";
import { TweetFindManyArgs } from "../../tweet/base/TweetFindManyArgs";
import { Tweet } from "../../tweet/base/Tweet";
import { TweetWhereUniqueInput } from "../../tweet/base/TweetWhereUniqueInput";

export class HashtagGrpcControllerBase {
  constructor(protected readonly service: HashtagService) {}
  @common.Post()
  @swagger.ApiCreatedResponse({ type: Hashtag })
  @GrpcMethod("HashtagService", "createHashtag")
  async createHashtag(
    @common.Body() data: HashtagCreateInput
  ): Promise<Hashtag> {
    return await this.service.createHashtag({
      data: data,
      select: {
        createdAt: true,
        id: true,
        metadata: true,
        name: true,
        updatedAt: true,
      },
    });
  }

  @common.Get()
  @swagger.ApiOkResponse({ type: [Hashtag] })
  @ApiNestedQuery(HashtagFindManyArgs)
  @GrpcMethod("HashtagService", "hashtags")
  async hashtags(@common.Req() request: Request): Promise<Hashtag[]> {
    const args = plainToClass(HashtagFindManyArgs, request.query);
    return this.service.hashtags({
      ...args,
      select: {
        createdAt: true,
        id: true,
        metadata: true,
        name: true,
        updatedAt: true,
      },
    });
  }

  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: Hashtag })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("HashtagService", "hashtag")
  async hashtag(
    @common.Param() params: HashtagWhereUniqueInput
  ): Promise<Hashtag | null> {
    const result = await this.service.hashtag({
      where: params,
      select: {
        createdAt: true,
        id: true,
        metadata: true,
        name: true,
        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: Hashtag })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("HashtagService", "updateHashtag")
  async updateHashtag(
    @common.Param() params: HashtagWhereUniqueInput,
    @common.Body() data: HashtagUpdateInput
  ): Promise<Hashtag | null> {
    try {
      return await this.service.updateHashtag({
        where: params,
        data: data,
        select: {
          createdAt: true,
          id: true,
          metadata: true,
          name: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: Hashtag })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("HashtagService", "deleteHashtag")
  async deleteHashtag(
    @common.Param() params: HashtagWhereUniqueInput
  ): Promise<Hashtag | null> {
    try {
      return await this.service.deleteHashtag({
        where: params,
        select: {
          createdAt: true,
          id: true,
          metadata: true,
          name: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Get("/:id/tweets")
  @ApiNestedQuery(TweetFindManyArgs)
  @GrpcMethod("HashtagService", "findManyTweets")
  async findManyTweets(
    @common.Req() request: Request,
    @common.Param() params: HashtagWhereUniqueInput
  ): Promise<Tweet[]> {
    const query = plainToClass(TweetFindManyArgs, request.query);
    const results = await this.service.findTweets(params.id, {
      ...query,
      select: {
        content: true,
        createdAt: true,

        hashtag: {
          select: {
            id: true,
          },
        },

        id: true,
        updatedAt: true,

        user: {
          select: {
            id: true,
          },
        },
      },
    });
    if (results === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return results;
  }

  @common.Post("/:id/tweets")
  @GrpcMethod("HashtagService", "connectTweets")
  async connectTweets(
    @common.Param() params: HashtagWhereUniqueInput,
    @common.Body() body: TweetWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      tweets: {
        connect: body,
      },
    };
    await this.service.updateHashtag({
      where: params,
      data,
      select: { id: true },
    });
  }

  @common.Patch("/:id/tweets")
  @GrpcMethod("HashtagService", "updateTweets")
  async updateTweets(
    @common.Param() params: HashtagWhereUniqueInput,
    @common.Body() body: TweetWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      tweets: {
        set: body,
      },
    };
    await this.service.updateHashtag({
      where: params,
      data,
      select: { id: true },
    });
  }

  @common.Delete("/:id/tweets")
  @GrpcMethod("HashtagService", "disconnectTweets")
  async disconnectTweets(
    @common.Param() params: HashtagWhereUniqueInput,
    @common.Body() body: TweetWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      tweets: {
        disconnect: body,
      },
    };
    await this.service.updateHashtag({
      where: params,
      data,
      select: { id: true },
    });
  }
}
